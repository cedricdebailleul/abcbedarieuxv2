...
export const createCampaignSchema = z.object({
  ...
  scheduledAt: z.string().datetime("Date/heure invalide").optional(),

  eventIds: z
    .array(z.string().cuid())
    .max(50, "Trop d'événements sélectionnés")
    .optional(),
  placeIds: z
    .array(z.string().cuid())
    .max(50, "Trop de commerces sélectionnés")
    .optional(),
  postIds: z
    .array(z.string().cuid())
    .max(50, "Trop d'articles sélectionnés")
    .optional(),
  attachmentIds: z
    .array(z.string().cuid())
    .max(10, "Trop de pièces jointes")
    .optional(),
});
...
export const clickTrackingSchema = trackingParamsSchema.extend({
  url: z.string().url("URL invalide").max(2048, "URL trop longue"),
});
