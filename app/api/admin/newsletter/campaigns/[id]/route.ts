    ...
    try {
      ...
      const updatedCampaign = await prisma.newsletterCampaign.update({
        ...
        data: {
          ...
          ...(validatedData.eventIds && {
            includedEvents: validatedData.eventIds,
          }),
          ...(validatedData.placeIds && {
            includedPlaces: validatedData.placeIds,
          }),
          ...(validatedData.postIds && {
            includedPosts: validatedData.postIds,
          }),
          updatedAt: new Date(),
        },
        include: {
    ...
    try {
      ...
      if (!["DRAFT", "CANCELLED"].includes(campaign.status)) {
        return NextResponse.json(
          {
            error:
              "Seules les campagnes en brouillon ou annulées peuvent être supprimées",
          },
          { status: 400 }
        );
      }
