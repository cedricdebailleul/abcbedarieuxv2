    ...
    const stats = await withCache(
      ...
      async () => {
        ...
        const engagementRate =
          totalUsers > 0
            ? Math.round(((totalReviews + totalFavorites) / totalUsers) * 100) /
              100
            : 0;

        // Taux de conversion newsletter
        const newsletterConversionRate =
          totalUsers > 0
        ...
        return {
          ...
          content: {
            ...
            upcomingEvents,
            contentRatio:
              totalPosts + totalPlaces + totalEvents > 0
                ? Math.round(
                    ((publishedPosts + activePlaces + upcomingEvents) /
                      (totalPosts + totalPlaces + totalEvents)) *
                      100
                  )
                : 0,
          },
          ...
          moderation: {
            ...
            claimsRatio:
              totalClaims > 0
                ? Math.round((pendingClaims / totalClaims) * 100)
                : 0,
          },
          ...
          engagement: {
            ...
            reviewsPerUser:
              totalUsers > 0
                ? Math.round((totalReviews / totalUsers) * 100) / 100
                : 0,
          },
