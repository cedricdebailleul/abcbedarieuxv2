      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600">
            {favoritesData ? 
              `${favoritesData.pagination.total} établissement${favoritesData.pagination.total > 1 ? 's' : ''} sauvegardé${favoritesData.pagination.total > 1 ? 's' : ''}` 
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-6">
            Vous n'avez pas encore ajouté d'établissement à vos favoris.
          </p>
          <p className="text-gray-500 mb-6">
            Parcourez nos établissements et cliquez sur ❤️ pour les sauvegarder ici !
          </p>
          <Button asChild>
          ...
          {favoritesData.pagination.pages > 1 && (
            <div className="flex items-center justify-center space-x-2 mt-8">
              ...
              <div className="flex items-center space-x-1">
                {Array.from({ length: favoritesData.pagination.pages }, (_, i) => i + 1).map((pageNumber) => {
                  const isCurrentPage = pageNumber === currentPage;
                  const isNearCurrentPage = Math.abs(pageNumber - currentPage) <= 2;
                  const isFirstOrLast = pageNumber === 1 || pageNumber === favoritesData.pagination.pages;
